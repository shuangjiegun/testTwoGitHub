...
public class Test1 {
	...
	public void test() {
		try { 
			...
			ResultSet rs = stmt.executeQuery("select * from  jydb.SecuMain t3 "); 		
			while (rs.next()) { 		
			...
			//System.out.println("【证券内部编码】" + rs.getInt("InnerCode") +"【交易日】" + rs.getTimestamp("TradingDay") +"【最新单位净值（元）】" + rs.getDouble("UnitNV") +"【日回报率（%）】" + rs.getDouble("NVDailyGrowthRate") +"【本月以来回报率（%）】" + rs.getDouble("RRInSelectedMonth") +"【今年以来回报率（%）】" + rs.getDouble("RRSinceThisYear") +"【更新时间】" + rs.getTimestamp("UpdateTime")  );
			System.out.println("【证券内部编码】" + rs.getInt("InnerCode") +"【公司代码】" + rs.getInt("CompanyCode") +"【证券代码】" + rs.getString("SecuCode") +"【中文名称】" + rs.getString("ChiName") +"【中文名称缩写】" + rs.getString("ChiNameAbbr") );
